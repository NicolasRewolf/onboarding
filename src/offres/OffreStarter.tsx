...
import { ArrowRight, ArrowUpRight, Check, Mail, Phone, X } from "lucide-react";
...
function Hero() {
  return (
    <section id="top" className="relative overflow-hidden border-b-2 border-rw-black bg-rw-white">
      <div className="mx-auto grid max-w-6xl items-center gap-10 px-5 py-14 sm:px-8 sm:py-16 lg:grid-cols-12 lg:gap-14 lg:py-20">
        ...
        <div className="lg:col-span-7">
          ...
          <h1 className="mt-5 text-[clamp(2.4rem,6.2vw,5rem)] leading-[0.92]">
            Construire <br className="hidden sm:inline" />
            une marque qui <span className="text-rw-orange">dure</span>.
          </h1>

          <p className="mt-7 max-w-xl text-[16px] leading-relaxed text-rw-muted sm:text-[17px]">
            <b className="text-rw-black">Deux forfaits</b>, pensés pour les{" "}
            <b className="text-rw-black">fondateurs d'entreprises food, lifestyle et bien-être</b> qui veulent une
            identité à la hauteur de leurs ambitions. Stratégie et direction artistique réunies, portées par{" "}
            <Highlight>5+ ans d'expérience</Highlight> en design et développement de marque.
          </p>
...
/** Surlignage marqueur orange — pas d'encadré, juste un coup de surligneur sur le texte. */
function Highlight({ children }: { children: React.ReactNode }) {
  return (
    <span className="bg-[linear-gradient(transparent_55%,var(--rw-orange)_55%,var(--rw-orange)_94%,transparent_94%)] px-0.5 font-bold text-rw-black">
      {children}
    </span>
  );
}
...
function ForfaitTease({
  ...
  return (
    <a
      href="#forfaits"
      className="group relative block border-2 border-rw-black bg-rw-white p-7 transition-all hover:-translate-y-1 hover:shadow-[var(--shadow-hard)] sm:p-8"
    >
      <div className="flex items-baseline justify-between">
        <span className="font-mono text-[11px] uppercase tracking-[0.22em] text-rw-black/60">
          Forfait {number}
        </span>
        <ArrowUpRight
          aria-hidden
          className="size-4 text-rw-black/30 transition-colors group-hover:text-rw-black"
        />
      </div>

      <p className="mt-7 text-[clamp(1.8rem,3.5vw,2.6rem)] font-extrabold uppercase leading-[0.95] tracking-tight">
        {name}
      </p>
      <p className="mt-3 max-w-xs text-[13.5px] leading-relaxed text-rw-black/75">
        {tagline}
      </p>

      <div className="mt-8 flex items-baseline justify-between gap-3 border-t border-rw-black/15 pt-4">
        <span className="font-mono text-[11px] uppercase tracking-[0.22em] text-rw-black/60">
          {delay}
        </span>
...
function Approche() {
  return (
    <section id="approche" className="border-b-2 border-rw-black bg-rw-white">
...
function Pillar({ n, title, kicker, body }: { n: string; title: string; kicker: string; body: string }) {
  return (
    <div>
      <span className="font-mono text-[11px] uppercase tracking-[0.22em] text-rw-muted">— {n}</span>
...
function Forfaits() {
  ...
  return (
    <section id="forfaits" className="border-b-2 border-rw-black bg-rw-white">
      <div className="mx-auto max-w-6xl px-5 py-20 sm:px-8 sm:py-28">
        <div className="flex flex-wrap items-end justify-between gap-6">
          <div>
            <p className="rw-eyebrow text-rw-orange">Deux forfaits</p>
            <h2 className="mt-4 text-[clamp(2rem,5vw,3.6rem)]">Choisissez votre formule.</h2>
          </div>
            ...
            {FORFAITS.map((f) => (
              <button
                ...
                className={cn(
                  ...
                  active === f.id ? "bg-rw-black text-rw-white" : "bg-rw-white text-rw-black hover:bg-rw-paper-subtle",
                )}
              >
                {f.number} · {f.name}
              </button>
        ...
        {/* Aperçu de l'autre forfait, pour comparer d'un œil */}
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          {FORFAITS.map((f) => (
            <button
              ...
              onClick={() => setActive(f.id)}
              className={cn(
                "flex items-center justify-between border-2 border-rw-black px-4 py-3 text-left transition-all",
                active === f.id
                  ? "bg-rw-orange text-rw-black"
                  : "bg-rw-white hover:translate-x-[2px] hover:translate-y-[2px]",
              )}
            >
              <span className="flex items-center gap-3">
                <span className="font-mono text-[10px] uppercase tracking-[0.22em] text-rw-muted">
                  Forfait {f.number}
                </span>
        ...
        <div className="grid grid-cols-2 md:col-span-5 md:grid-cols-1">
          <div className="border-b-2 border-rw-black bg-rw-orange p-5 sm:p-6">
            ...
            <p className="mt-2 text-[44px] font-extrabold leading-none tracking-tight">
              ...
              <span className="ml-1 text-base font-bold">€ HT</span>
            </p>
          </div>
...
function Comparatif() {
  ...
  return (
    <section id="comparatif" className="border-b-2 border-rw-black bg-rw-white">
      <div className="mx-auto max-w-6xl px-5 py-20 sm:px-8 sm:py-28">
        <p className="rw-eyebrow text-rw-orange">Comparatif</p>
        <h2 className="mt-4 max-w-3xl text-[clamp(2rem,5vw,3.6rem)]">
          Quelle formule vous <span className="text-rw-orange">correspond</span> ?
        </h2>

        <div className="mt-12 border-2 border-rw-black bg-rw-white shadow-[var(--shadow-hard)]">
          <div className="grid grid-cols-[1fr_auto_auto] gap-x-4 border-b-2 border-rw-black bg-rw-black px-5 py-3 text-rw-white sm:px-7">
            <span className="font-mono text-[11px] uppercase tracking-[0.22em] text-rw-white/60">
              Éléments inclus
            </span>
            <span className="w-20 text-center font-mono text-[11px] uppercase tracking-[0.22em] sm:w-28">
              Origine
            </span>
            <span className="w-20 text-center font-mono text-[11px] uppercase tracking-[0.22em] text-rw-orange sm:w-28">
              Signature
            </span>
          </div>
          ...
          <ul>
            {COMPARATIF.map((row, i) => (
              <li
                ...
                className={cn(
                  "grid grid-cols-[1fr_auto_auto] items-center gap-x-4 border-b border-rw-line-subtle px-5 py-3.5 text-[14.5px] transition-colors last:border-b-0 sm:px-7",
                  hover === i && "bg-rw-orange/10",
                )}
              >
                <span>{row.label}</span>
                <span className="w-20 text-center sm:w-28">
                  <Tick on={row.origine} />
                </span>
                <span className="w-20 text-center sm:w-28">
            ...
            <li className="grid grid-cols-[1fr_auto_auto] items-center gap-x-4 bg-rw-black px-5 py-4 text-rw-white sm:px-7">
              <span className="font-mono text-[11px] uppercase tracking-[0.22em] text-rw-white/60">Tarif HT</span>
              <span className="w-20 text-center font-mono text-sm font-bold sm:w-28">800 €</span>
              <span className="w-20 text-center font-mono text-sm font-bold text-rw-orange sm:w-28">1 600 €</span>
            </li>
          </ul>
        </div>
      </div>
...
function DevisBuilder() {
  const [forfaitId, setForfaitId] = useState<Forfait["id"]>("origine");
  ...
  const total = forfait.price + extrasTotal;
  ...
  return (
    <section id="extras" className="border-b-2 border-rw-black bg-rw-white">
      <div className="mx-auto max-w-6xl px-5 py-20 sm:px-8 sm:py-28">
        ...
        <p className="mt-4 max-w-2xl text-[15px] text-rw-muted">
          Estimation indicative HT. Le devis final est confirmé après l'appel découverte.
        </p>
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-3">
          ...
          <aside className="lg:col-span-1">
            <div className="sticky top-20 border-2 border-rw-black bg-rw-black p-6 text-rw-white shadow-[var(--shadow-hard-orange)]">
              <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-rw-orange">Estimation</p>
              <h3 className="mt-3 text-rw-white">Votre devis</h3>

              <dl className="mt-6 space-y-2.5 border-y-2 border-rw-white/15 py-5 font-mono text-[12.5px]">
                <div className="flex justify-between text-rw-white/80">
                  <dt>Forfait {forfait.name}</dt>
                  <dd className="font-bold text-rw-white">{formatPrice(forfait.price)} €</dd>
                </div>
                {EXTRAS.filter((e) => picked.has(e.id)).map((e) => (
                  <div key={e.id} className="flex justify-between text-rw-white/80">
                    <dt>+ {e.name}</dt>
                    <dd className="font-bold text-rw-white">{formatPrice(e.price)} €</dd>
                  </div>
                ))}
                {picked.size === 0 && (
                  <p className="text-rw-white/50">Aucun extra sélectionné.</p>
                )}
              </dl>

              <div className="mt-5 flex items-baseline justify-between">
                <span className="font-mono text-[11px] uppercase tracking-[0.22em] text-rw-white/60">Total HT</span>
                <span className="text-[38px] font-extrabold tracking-tight text-rw-orange">
                  {formatPrice(total)}
                  <span className="ml-1 text-base font-bold text-rw-white">€</span>
                </span>
              </div>

              <Button variant="rw" size="lg" asChild className="mt-6 w-full">
                <a
                  href={`mailto:${CONTACT.email}?subject=${encodeURIComponent(
                    `Demande de devis — Forfait ${forfait.name}`,
                  )}&body=${encodeURIComponent(devisMailBody(forfait, picked, total))}`}
                >
                  Envoyer ce devis <ArrowRight className="size-5" />
                </a>
              </Button>
              <p className="mt-3 text-center font-mono text-[10px] uppercase tracking-[0.22em] text-rw-white/40">
...
function devisMailBody(forfait: Forfait, picked: Set<string>, total: number): string {
  const lines = [
    "Bonjour Elise,",
    "",
    `Je souhaite un devis pour le forfait ${forfait.name} (${formatPrice(forfait.price)} € HT).`,
  ];
  ...
  lines.push("", `Total estimé : ${formatPrice(total)} € HT.`, "", "À très vite,");
  return lines.join("\n");
}
...
function Process() {
  return (
    <section id="process" className="border-b-2 border-rw-black bg-rw-white">
      <div className="mx-auto max-w-6xl px-5 py-20 sm:px-8 sm:py-28">
...
function Portfolio() {
  return (
    <section className="border-b-2 border-rw-black bg-rw-black text-rw-white">
      <div className="mx-auto max-w-6xl px-5 py-20 sm:px-8 sm:py-28">
        <div className="flex flex-wrap items-end justify-between gap-6">
          <div>
            <p className="rw-eyebrow text-rw-orange">Portfolio</p>
            <h2 className="mt-4 max-w-3xl text-rw-white text-[clamp(1.8rem,4.5vw,3rem)]">
              Quelques marques <span className="text-rw-orange">façonnées</span>.
            </h2>
          </div>
          <a
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {PORTFOLIO.map((p) => (
            ...
            >
              <div
                className="aspect-[4/5] border-2 border-rw-white bg-rw-paper-subtle bg-cover bg-center transition-transform group-hover:-translate-y-1"
                style={{ backgroundImage: `url('/projects/${p.slug}.jpg')` }}
                role="img"
                aria-label={`Aperçu de l'identité ${p.name}`}
              />
              <div className="mt-4 flex items-baseline justify-between gap-3">
                <h3 className="text-lg text-rw-white">{p.name}</h3>
                <span className="font-mono text-[11px] uppercase tracking-[0.22em] text-rw-orange">
                  {p.tag}
                </span>
              </div>
              <p className="mt-1 text-[13px] text-rw-white/60">{p.detail}</p>
            </a>
          ))}
        </div>
      </div>
...
function Contact() {
  return (
    <section id="contact" className="border-b-2 border-rw-black bg-rw-white">
      <div className="mx-auto grid max-w-6xl gap-12 px-5 py-20 sm:px-8 sm:py-28 lg:grid-cols-12">
        <div className="lg:col-span-7">
          <p className="rw-eyebrow text-rw-orange">Prêt·e à poser les bases ?</p>
          <h2 className="mt-5 text-[clamp(2.4rem,7vw,5.4rem)] leading-[0.9]">
            On a hâte <br />
            d'entendre parler <br />
            <span className="text-rw-orange">de vous.</span>
          </h2>
        ...
        {/* Fiche contact personnelle — seule mention nominative d'Elise. */}
        <aside className="lg:col-span-5">
          <div className="border-2 border-rw-black bg-rw-white p-6 shadow-[var(--shadow-hard)]">
            <p className="font-mono text-[10px] uppercase tracking-[0.22em] text-rw-orange">Votre interlocutrice</p>
            <p className="mt-3 text-2xl font-extrabold uppercase tracking-tight">{CONTACT.name}</p>
            <p className="mt-2 text-[14px] text-rw-muted">{CONTACT.role}</p>
            <p className="mt-1 text-[13px] text-rw-tertiary">{CONTACT.credentials}</p>

            <dl className="mt-6 space-y-2 border-t-2 border-rw-black/15 pt-5 font-mono text-[12px]">
              <div className="flex justify-between gap-4">
                <dt className="text-rw-muted">E-mail</dt>
                <dd className="truncate text-rw-black">{CONTACT.email}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-rw-muted">Téléphone</dt>
                <dd className="text-rw-black">{CONTACT.phone}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-rw-muted">Site</dt>
                <dd className="text-rw-black">{CONTACT.site}</dd>
              </div>
            </dl>
          </div>
        </aside>
...
/* ─────────────────────────── Final CTA (bande orange) ─────────────────────────── */

function FinalCTA() {
  return (
    <section className="border-b-2 border-rw-black bg-rw-orange">
      <div className="mx-auto flex max-w-6xl flex-col items-start justify-between gap-8 px-5 py-16 sm:px-8 sm:py-20 lg:flex-row lg:items-center">
        <div>
          <p className="rw-eyebrow text-rw-black/70">Notre travail</p>
          <h2 className="mt-4 text-rw-black text-[clamp(2rem,5.5vw,4rem)] leading-[0.95]">
            Découvrez tous nos projets <br className="hidden md:inline" />
            sur rewolf.studio.
          </h2>
        </div>
        <Button variant="rwDark" size="lg" asChild>
          <a href="https://rewolf.studio" target="_blank" rel="noreferrer">
            Visiter le site <ArrowUpRight className="size-5" />
          </a>
        </Button>
      </div>
    </section>
